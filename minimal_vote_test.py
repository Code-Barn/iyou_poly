#!/usr/bin/env python
"""
Minimal test for voting functionality in the Poller app.
...
def create_test_data():
    ...
    poll = Poll.objects.create(
        title="Minimal Test Poll",
        description="A minimal test poll for voting functionality",
        created_by=user,
...
def test_voting():
    ...
    # Vote for the option using the minimal vote view
    url = reverse("minimal_vote", args=[poll.id])
    ...
    if vote_count == 1 and total_votes == 1:
        ...
        if (
            f"You voted for: <strong>{option.text}</strong>"
            in detail_response.content.decode("utf-8")
        ):
...
if __name__ == "__main__":
    print("Running minimal voting test...")
    success = test_voting()

    if success:
        print("\n🎉 Voting functionality is working correctly!")
        sys.exit(0)
    else:
        print("\n❌ Voting functionality is not working correctly.")
        sys.exit(1)
