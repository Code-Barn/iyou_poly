...
def test_vc_labeling():
    ...
    test_cases = [
        ...
        {"vc": {"type": ["VerifiableCredential"]}, "expected": "User Credential 1"},
        {
    ...
    for i, test_case in enumerate(test_cases):
        ...
        if vc.get("type") and "AuthenticationCredential" in vc["type"]:
            ...
            label = f"User Credential {i + 1}"

        if label == expected:
            logger.info(f"✓ Test case {i + 1}: Correct label '{label}'")
        else:
            logger.error(f"✗ Test case {i + 1}: Expected '{expected}', got '{label}'")
            return False
