...
Must be explicitly declared in poll metadata (e.g., "vote_power_rule": "investor_share").
UI must visually indicate non-1:1 ratios (e.g., gear icon ⚙️ with tooltip).
Supported custom rules:

investor_share (e.g., 0.5x for partial owners)
...
REST API Endpoints:
      ...
      /polls
      POST
      Create a poll (supports custom ratios).
    
    
      /polls/{id}/vote
      POST
      Cast a vote (DID-signed).
    
    
      /polls/{id}/verify
      GET
      Verify poll results (returns Merkle root).
    
    
      /polls/{id}/rules
      GET
      Get vote power rules for a poll.
